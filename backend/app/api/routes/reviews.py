...
async def api_review_state(request: Request):
    ...
    return {"success": True, "data": backend.engine.seller_reviews.get_state()}
...
async def api_review_state_replace(request: Request):
    ...
    try:
        data = await asyncio.get_event_loop().run_in_executor(None, backend.engine.seller_reviews.replace_state, body)
